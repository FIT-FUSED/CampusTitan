...
function getExerciseRecommendation(envData) {
    if (!envData) return { text: 'No data available', emoji: '❓', safe: true };
    const { aqi, temperature, humidity } = envData;
...
export default function EnvironmentScreen({ navigation }) {
    ...
    const recommendation = getExerciseRecommendation(today);
